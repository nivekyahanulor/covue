...
<head>
  ...
  <link rel="stylesheet" href="<?php echo base_url(); ?>plugins/bootstrap-tour/css/bootstrap-tourist.css">
  ...
  <script>
    ...
    gtag('config', 'G-VKXPXN6X1T', {cookie_flags: 'SameSite=None;Secure'});
  </script>
  ...
  <script>
    ...
    // var pusher = new Pusher('d9fa3c2060a75841885a', {
    //   cluster: 'ap1'
    // });

    // var channel = pusher.subscribe('my-channel');
    // channel.bind('my-event'+<?php //echo $this->session->userdata('user_id'); ?>, function(data) {
    //    var badge =  $(".navbar-badge").text();
    //    if ($('.navbar-badge').text() == ''){
    //       $(".navbar-badge").html('1');
    //    } else {
    //       $(".navbar-badge").html(parseInt(badge) + 1);

    //    }
    //    $(".regulated-status").append('<a href="tracking_application/'+data.id+'" class="dropdown-item"><i class="fas fa-envelope mr-2"></i> '+data.message+' <span class="float-right text-muted text-sm"></span></a>');
    // });
  </script>
</head>
